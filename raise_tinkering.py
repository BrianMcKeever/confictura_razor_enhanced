...
while Player.GetRealSkillValue("Tinkering") < Player.GetSkillCap("Tinkering"):
    ...
    if not boards or boards.Amount < 10:
        ingots = Items.FindByID(0x1BF2, 0, source_container)
    ...
    if Player.GetSkillValue("Tinkering") < 0 + 50: #NOT SURE ABOUT 50
        ...
        continue
    if Player.GetSkillValue("Tinkering") < 10 + 50:
        Gumps.WaitForGump(949095101, 10000) 
        ...
        Gumps.WaitForGump(949095101, 10000)
        Gumps.SendAction(949095101, 0)
        print(len(Player.Backpack.Contains))
        if len(Player.Backpack.Contains) > 100:
            tool = get_tinker_tool()
            Items.Move(tool, trash_barrel, 1)
            Misc.Pause(1000)
        continue
